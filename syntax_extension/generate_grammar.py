#!/usr/bin/env python3
"""
Generates mtc.tmLanguage.json from the C++ tokenizer KEYWORDS set.
...
TOKENIZER_PATH = SCRIPT_DIR.parent / "src" / "tokenizer.cpp"
...
TYPES = {"int", "float", "void", "array", "dict", "string", "bool"}
CONTROL_FLOW = {"if", "elif", "else", "for", "while", "return", "in", "set", "break", "try", "catch", "throw"}
IMPORTS = {"from", "use", "as"}
CONSTANTS = {"true", "false"}
CLASS_KEYWORDS = {"func", "class", "new", "this", "static", "virtual", "dynamic"}
# Everything else will be treated as builtins/other keywords


def read_keywords_from_tokenizer():
    """Parse KEYWORDS set from src/tokenizer.cpp"""
    ...
    if not match:
        raise ValueError("Could not find KEYWORDS set in src/tokenizer.cpp")
